...
function leerNumero(valor: any): number {
  ...
  const str = String(valor).replace(/\$/g, '').replace(/\s/g, '')
...
export async function parseExcelNomina(archivo: File): Promise<any[]> {
  ...
  // Detectar automaticamente la fila donde estan los encabezados reales
  const filasRaw: any[][] = XLSX.utils.sheet_to_json(hoja, { header: 1, defval: '' })
  let filaEncabezado = 0
  const CLAVES = ['nombre','empleado','cedula','identificacion','basico','sueldo','salario','neto','transporte']
  for (let i = 0; i < Math.min(filasRaw.length, 20); i++) {
    const texto = filasRaw[i].join(' ').toLowerCase()
  ...
  const filasCrudas: any[] = XLSX.utils.sheet_to_json(hoja, {
    defval: '', raw: false, range: filaEncabezado
  })
  ...
  const colCedula = detectarColumna(encabezados, ['cedula','cc','identificacion','documento','id empleado','nro documento','numero documento','doc','identificador','nit','dni','identificacion no'])
  ...
  const colArea = detectarColumna(encabezados, ['area','cargo','puesto','departamento','seccion','sede','lugar','centro costo','centro de costo','ubicacion','trabajo','area trabajo'])
  const colBasico = detectarColumna(encabezados, ['basico','sueldo','salario','pago base','salario base','sueldo base','salario basico','basico mensual','salario mensual','sueldo basico'])
  ...
  for (const fila of filasCrudas) {
    ...
    filas.push({
      ...
      conceptos: { sueldoBase: basico, auxilioTransporte: transporte, bonificaciones: bonos, prima, vacaciones, prestamo, descuento, abonoPrima, cesantias, abonoCesantias, abonoLiquidacion },
...
export async function guardarNominaProgramada(
  filas: any[], userId: string, periodoContable: string
): Promise<{
  filasInsertadas: number; filasActualizadas: number
  ...
  for (const fila of filas) {
    try {
      ...
      const registro = {
        user_id: userId, nombre_empleado: fila.nombre, cedula: fila.cedula,
        area: fila.area || null, sueldo_base: fila.conceptos.sueldoBase,
      ...
      const { data: existente } = await supabase
        .from('nomina_programada').select('id, estado')
        .eq('user_id', userId).eq('cedula', fila.cedula)
        .eq('periodo_contable', periodoContable).maybeSingle()

      if (existente) {
        const estadoFinal = existente.estado === 'Pagado' ? 'Pagado' : 'Pendiente de Pago'
        const { error } = await supabase.from('nomina_programada')
          .update({ ...registro, estado: estadoFinal }).eq('id', existente.id)
