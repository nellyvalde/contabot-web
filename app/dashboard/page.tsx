...
const generarDesprendible = () => {
  ...
  const html = `<html><head><title>Desprendible - ${n.nombre_empleado}</title>
    ...
    <h2>💰 Devengados</h2>
    ${n.sueldo_base>0?'<div class="row"><span>Sueldo Básico</span><span>$'+fmt(n.sueldo_base)+'</span></div>':''}
    ${n.auxilio_transporte>0?'<div class="row"><span>Aux. Transporte</span><span>$'+fmt(n.auxilio_transporte)+'</span></div>':''}
    ${n.bonificaciones>0?'<div class="row"><span>Bonificaciones</span><span>$'+fmt(n.bonificaciones)+'</span></div>':''}
    ${n.abono_prima>0?'<div class="row"><span>Abono Prima</span><span>$'+fmt(n.abono_prima)+'</span></div>':''}
    ${n.saldo_anterior>0?'<div class="row"><span>Saldo Mes Anterior</span><span>$'+fmt(n.saldo_anterior)+'</span></div>':''}
    ${n.pension>0||n.salud>0?'<h2>➖ Deducciones</h2>':''}
    ${n.pension>0?'<div class="row"><span>Pensión (4%)</span><span>-$'+fmt(n.pension)+'</span></div>':''}
    ${n.salud>0?'<div class="row"><span>Salud (4%)</span><span>-$'+fmt(n.salud)+'</span></div>':''}
    <div class="row total"><span>NETO A PAGAR</span><span class="neto">$${fmt(n.neto_pagar)}</span></div>
    ...
    <>
      <tr className="border-b last:border-0 hover:bg-slate-50 cursor-pointer" onClick={() => setExpandido(!expandido)}>
        ...
        <td className="py-2 text-right pr-4 font-semibold text-emerald-600">
          ${Math.round(n.neto_pagar || 0).toLocaleString()}
        </td>
        <td className="py-2">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-200">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-emerald-600 text-base">${Math.round(n.neto_pagar || 0).toLocaleString()}</span>
            </div>
