...
export default function CobrarPage() {
  ...
  const facturasCobrar = facturas.filter(f => {
    ...
    if (filtroCliente && !f.proveedor?.toLowerCase().includes(filtroCliente.toLowerCase())) return false
    if (filtroEstado && f.estado !== filtroEstado) return false
    return true
  })

  const total = facturasCobrar.filter(f => f.estado === 'Pendiente').reduce((a, b) => a + (b.valor || 0), 0)
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 ml-64 p-8">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm mb-6 border-l-4 border-green-400">
          ...
          <p className="text-3xl font-bold text-green-600 mt-1">${Math.round(total).toLocaleString()}</p>
        </div>
        <div className="bg-white rounded-2xl p-4 shadow-sm mb-4">
          <div className="grid grid-cols-2 gap-4">
            <input type="text" placeholder="Buscar cliente..." value={filtroCliente} onChange={e => setFiltroCliente(e.target.value)} className="px-4 py-2 border border-slate-200 rounded-xl text-sm" />
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          {facturasCobrar.length === 0 ? (
            <p className="text-center py-10 text-slate-400">No hay facturas de venta.</p>
          ) : (
            <table className="w-full text-sm">
              <thead><tr className="text-left text-slate-500 border-b">
                <th className="pb-2">Cliente</th><th className="pb-2">Valor</th><th className="pb-2">Estado</th><th className="pb-2"></th>
              </tr></thead>
              <tbody>{facturasCobrar.map(f => (
                <tr key={f.id} className="border-b last:border-0 hover:bg-slate-50">
                  <td className="py-3 font-medium">{f.proveedor}</td>
                  <td className="py-3 text-emerald-700 font-medium">${Math.round(f.valor||0).toLocaleString()}</td>
      ...
      {pagoModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 w-full max-w-md shadow-2xl">
            ...
            <p className="mb-6 text-slate-600">Cliente: <strong>{pagoModal.proveedor}</strong></p>
            <div className="flex gap-3">
              <button onClick={() => setPagoModal(null)} className="flex-1 px-4 py-2 border rounded-xl text-sm">Cancelar</button>
              <button onClick={handleRegistrarPago} className="flex-1 px-4 py-2 bg-emerald-500 text-white rounded-xl text-sm">Confirmar</button>
            </div>
          </div>
        </div>
      )}
