      ...
      <main className="flex-1 ml-64 p-8">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          {clientesAgrupados.length === 0 ? (
            <div className="text-center py-10 text-slate-400"><p>No hay clientes aún.</p></div>
          ) : (
            <table className="w-full text-sm">
              <thead><tr className="text-left text-slate-500 border-b">
                <th className="pb-2">Cliente</th><th className="pb-2">Facturas</th>
                <th className="pb-2">Total Facturado</th><th className="pb-2">Pendiente por Cobrar</th>
                <th className="pb-2">Ultima Factura</th><th className="pb-2">Estado</th>
              </tr></thead>
              <tbody>
                {clientesAgrupados.map((c) => (
                  <tr key={c.nombre} className="border-b last:border-0 hover:bg-slate-50">
                    ...
                    <td className="py-3"><span className={`font-medium ${c.totalPendiente > 0 ? 'text-yellow-600' : 'text-green-600'}`}>${Math.round(c.totalPendiente).toLocaleString()}</span></td>
                    <td className="py-3 text-slate-500">{c.ultimaFactura}</td>
                    <td className="py-3"><span className={`px-2 py-1 rounded-full text-xs font-medium ${c.totalPendiente > 0 ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}`}>{c.totalPendiente > 0 ? 'Pendiente' : 'Al día'}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
