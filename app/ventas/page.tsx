...
export default function VentasPage() {
  ...
  async function cargarDocumentos() {
    ...
    const { data } = await supabase
      .from('documentos')
      .select('id, numero_documento, proveedor_cliente, descripcion, valor, iva, fecha_emision, estado, estado_conciliacion, cuenta_puc, archivo_url')
  ...
  const totalFacturado = documentos.reduce((s, d) => s + Number(d.valor ?? 0), 0)
        ...
        <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              ...
              <div className="flex gap-3">
                <div className="rounded-xl bg-emerald-50 border border-emerald-100 px-4 py-3">
                  ...
                  <p className="text-xl font-bold text-emerald-700">${totalFacturado.toLocaleString()}</p>
                </div>
          ...
          <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-semibold text-slate-900">Historial de Ventas</h2>
              <input value={buscar} onChange={e => setBuscar(e.target.value)}
                placeholder="Buscar cliente o N° documento..."
            ...
            {cargando ? (
              ...
              <div className="p-8 text-center">
                <p className="text-slate-500 font-medium">Sin facturas de venta</p>
                ...
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-100">
                      {['Nº Documento','Cliente','Fecha','Valor','IVA','Cuenta PUC','Estado','PDF'].map(h => (
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {filtrados.map(doc => (
                      <tr key={doc.id} className="hover:bg-slate-50/70">
                        ...
                        <td className="px-4 py-3.5 font-semibold text-emerald-700">${Number(doc.valor ?? 0).toLocaleString()}</td>
